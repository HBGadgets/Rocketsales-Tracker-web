...
const CheckInCheckOut = () => {
  ...
  const fetchData = async (startDate, endDate, selectedPeriod, page = 1) => {
    ...
    try {
      ...
      if (response.data) {
        // Map initial data without waiting for address resolution
        const initialData = response.data.data.map((item) => ({
          ...item,
          createdAt: item.createdAt ? formatDateTime(item.createdAt) : null,
          checkOutTime: item.checkOutTime ? formatDateTime(item.checkOutTime) : null,
          ...
          startLat: item.startLat || 'N/A',
          startLong: item.startLong || 'N/A',
          endLat: item.endLat || 'N/A',
          endLong: item.endLong || 'N/A',
          CHECKINaddress: "Fetching...",  // Placeholder
          CHECKOUTaddress: "Fetching...", // Placeholder
        }))
        ...
        // Fetch addresses in the background
        const updatedData = await Promise.all(
          initialData.map(async (item) => {
            const checkInAddress = await fetchAddress(item.startLat, item.startLong)
            const checkOutAddress = await fetchAddress(item.endLat, item.endLong)
  
            return {
              ...item,
              CHECKINaddress: checkInAddress || "Not Found",
              CHECKOUTaddress: checkOutAddress || "Not Found",
            }
          })
        )
  ...
  const formatDateTime = (isoString) => {
    ...
    return new Intl.DateTimeFormat('en-US', {
      ...
      second: '2-digit',
      hour12: true,
      timeZone: 'UTC', // Keeps the time in UTC without converting to local timezone
    }).format(date)
  }
  
  const API_KEY = 'Iu2463vlcTCeu4XARM0O'

  // const fetchAddress = async (lat, long) => {
  //   try {
  //     const url = `https://api.maptiler.com/geocoding/${long},${lat}.json?key=${API_KEY}`
  //     const response = await axios.get(url)
  // console.log("res add",response.data)
  //     if (response.data && response.data.features.length > 0) {
  //       return response.data.features[0].place_name_en 
  //       // Extracts formatted address
  //     } else {
  ...
  const fetchAddress = async (lat, long) => {
    try {
      ...
      if (response.data && response.data.features.length > 0) {
        ...
        // Extracting a more detailed address
        const addressParts = feature.context ? feature.context.map(c => c.text) : []
        const fullAddress = [feature.place_name, ...addressParts].filter(Boolean).join(', ')
  ...
  const exportToExcel = ExcelExporter({
    mytitle: 'CheckIn And CheckOut Data Report',
    columns: COLUMNS().slice(1),
    data: filteredData.map(({ [COLUMNS()[0]?.accessor]: _, ...rest }) => rest),
    fileName: 'CheckInAndCheckOut_report.xlsx',
  })

  const exportToPDF = PDFExporter({
    title: 'CheckIn And CheckOut Data Report',
    columns: COLUMNS().slice(1),
    data: filteredData.map(({ [COLUMNS()[0]?.accessor]: _, ...rest }) => rest),
    fileName: 'CheckInAndCheckOut_report.pdf',
  })
